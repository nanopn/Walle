...
try:
    ...
    class Microphone(AudioSource):
        def __init__(self, device_index = None):
            self.device_index = 2 # device_index
            self.format = pyaudio.paInt16 # 16-bit int sampling
            self.SAMPLE_WIDTH = pyaudio.get_sample_size(self.format)
            self.RATE = 44100 #22050 # sampling rate in Hertz
            self.CHANNELS = 1 # mono audio
            self.CHUNK = 1024  #612 in wall # number of frames stored in each buffer
...
class Recognizer(AudioSource):
    def __init__(self, language = "es-MX", key = "AIzaSyBOti4mM-6x9WDnZIjIeyEU21OpBXqWBgw"):
        self.key = key
        self.language = language

        self.energy_threshold = 3700  # minimum audio energy to consider for recording
